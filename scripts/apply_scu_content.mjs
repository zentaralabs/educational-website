...
// english_requirements, source_url) to Southern Cross University published
// programs, sourced live from scu.edu.au (course-finder pages) and, for the two
// HDR rows, the Graduate School PhD / Masters by Thesis pages, during the
// 2026-09-19/20 build-out sprint.
//
//   node scripts/apply_scu_content.mjs scripts/data/scu_updates.json          # dry run
//   node scripts/apply_scu_content.mjs scripts/data/scu_updates.json --commit
...
if (gapRows.length) {
  console.log(`${gapRows.length} row(s) have a genuine admission_requirements gap (left null, documented owed):`, gapRows.map((r) => r.name));
}
