...
// Archive AIM program rows with cited evidence, writing BOTH status='archived'
// and discontinued_note (a past bug in archive_programs.mjs silently dropped
// discontinued_note from writes - always archive last, per repo convention).
//
//   node scripts/aim_archive.mjs scripts/data/aim_archive_list.json          # dry run
//   node scripts/aim_archive.mjs scripts/data/aim_archive_list.json --commit
//
...
const logFile = path.join("scripts", "data", "aim-archived.json");
...
for (const it of items) {
  ...
  console.log(`  ${COMMIT ? "archived" : "would archive"}: ${rows[0].name} (${rows[0].status}) - ${it.reason.slice(0, 80)}...`);
  if (COMMIT) {
    await c.query(
