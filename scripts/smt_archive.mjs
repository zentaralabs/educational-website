...
// Archive South Metropolitan TAFE program rows with cited evidence, writing BOTH status='archived'
// and discontinued_note (a past bug in archive_programs.mjs silently dropped
// discontinued_note from writes - see scripts/data/programs-dash-cleanup memory).
//
//   node scripts/tafe_archive.mjs scripts/data/tafe_archive_list.json          # dry run
//   node scripts/tafe_archive.mjs scripts/data/tafe_archive_list.json --commit
//
...
const logFile = path.join("scripts", "data", "smt-archived.json");
