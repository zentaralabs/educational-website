...
- Resume: demand tier for the 4 bulk-catalog schools (Bond/Adelaide/Murdoch/Canberra bachelor + master coursework degrees, incl. Bond's Professional twins) is now fully covered. The 212 remaining empty stubs are all the honest-minimal-card tier: grad certs/diplomas, honours-only rows, PhD/MPhil research degrees, foundation/pathway programs, and a handful of Adelaide degrees whose DB name maps to no single course page. These stay as the minimal card unless a specific one is prioritised. To pick one up: `node scripts/program_descriptions_pull.mjs "<University>"` → write `scripts/data/_w.json` as `[{id, description, source_url, duration_years?}]` → `node scripts/program_descriptions_apply.mjs scripts/data/_w.json --commit` → `node scripts/export_programs.mjs` → POST /api/revalidate for `programs` → commit.
