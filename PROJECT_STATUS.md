...
- `git push` is failing with **403 "denied to LamaRoman"** (token expired / access changed mid-session on 2026-08-29). Commit `72f7729` is safe locally. Once auth is restored (`gh auth login` or refresh credential), push it, wait for deploy, then: verify `/sitemap.xml` program count went 790 -> ~868, `POST /api/revalidate` for `programs`, and **resubmit `/sitemap.xml` in both GSC (property under romanlama314@gmail.com) and Bing** (both were resubmitted on 2026-08-29 for the 790-URL version; GSC showed "Couldn't fetch" — likely stale from deploy churn, sitemap verified valid via xmllint).
