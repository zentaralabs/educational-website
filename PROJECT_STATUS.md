...
- **NEXT STEP — un-hide wave (user approved "after all descriptions are done", which is now):** program pages are still `robots: noindex, follow` ([page.tsx:71](src/app/(site)/universities/[slug]/programs/[programId]/page.tsx:71)) and excluded from the sitemap ([sitemap.ts:175](src/app/sitemap.ts:175)). To un-hide: gate the `robots` metadata and the sitemap `programEntries` on `hasDescription` (all launched-uni published programs now qualify), add the program entries back to `sitemap.ts` via `listPublishedProgramsForSitemap` (already in `public-programs.ts`), and remove the noindex. Then submit the updated sitemap. This is the remaining task on the program-pages workstream.
