...
1. **Country-of-origin pages** — TEMPLATE LIVE 2026-08-29: `/international/[country]` + `/international` hub, config in `src/lib/origin-countries.ts` (same pattern as collections/subjects/cities). First page: `/international/india` — agent-vs-direct rules, earlier deadlines for higher-scrutiny countries, 3-year-bachelor's recognition, English/MOI, visa 500 (fee now A$2,500, confirmed against immi.homeaffairs), Genuine Student, 485->PR. Sections + FAQ schema + LastVerified + RelatedLinks. Awaiting user review before adding Nepal / Pakistan / China / Vietnam / Bangladesh / Sri Lanka.
