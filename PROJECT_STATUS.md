...
- **Long-tail pass IN PROGRESS (2026-08-29):** user wants the ~212 remaining stubs described too, but "in the best interest of AdSense approval, not filling AI content blindly" — so: family-templated batches where the templated card carries real per-row substance (specific subjects, accreditation, honest limitations), decline to a minimal card where nothing genuine can be said, archive junk rows. Un-hide (drop noindex + add programs to sitemap) happens as ONE wave AFTER all descriptions are done, gated on description presence/quality.
  ...
  - Deferred/flagged: Murdoch "Master of Applied Psychology" row is ambiguous (source_url points at m1156 = the APAC-accredited *Clinical Psychology* master's, but there is also a separate Master of Applied Psychology (Professional) m1238) — left as minimal card pending a name fix.
  - Still to do: 3 Adelaide name-mismatch degrees (Music, Teaching, Engineering), 3 Canberra generic bachelors (Creative Industries, Secondary Education, Built Environment), research-degree templates (~15: PhD/MPhil/professional doctorates/"...by Research" master's), honours templates (~21), pathway/enabling real ones + archive junk (K-Track, OnTrack, FlexiTrack High, TLC, UC Connect etc.), then the 116 grad cert/diploma family cards.
