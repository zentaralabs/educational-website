...
- **Un-hide wave — DONE (2026-08-29), staged not blanket.** `isProgramIndexable(program)` in `public-programs.ts` gates both the page `robots` metadata and the new sitemap `programEntries`: index if the program has a parsed `curriculum` (the 596 fully-enriched) OR an "About this program" description of **110+ words** (194 more — the demand-tier degree cards + longer family cards). The **230 short templated cards (60-109 words)** — short one-year honours years, short grad-cert family cards, short pathway cards — stay `noindex, follow` and out of the sitemap, still live for users and internal links, pending a later verification wave. Result: **790 program URLs now in the sitemap** (was 0); total sitemap ~1,067 (was ~277). Verified in dev: 110+w and curriculum pages `index`, sub-110w pages `noindex` and absent from sitemap.
  - Rationale for staged: the user's whole concern this session was AdSense ("not filling AI content blindly"), and the application window is ~2026-09-18. 790 pages where the weakest is a 110+ word sourced summary with a citation is defensible; dumping all 1,020 including 60-word cards right before an AdSense review is the "scaled content" risk. To be more conservative, raise the word threshold in `isProgramIndexable` (or gate on `curriculum` only) — one-line change.
