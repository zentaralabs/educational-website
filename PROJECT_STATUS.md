...
**UPDATE (2026-08-29):** superseded by the full description pass — see "Description pass — COMPLETE" below. Every published program now has a sourced description; ~790 (the 596 enriched + 194 with 110+ word descriptions) are back in the sitemap and indexed via `isProgramIndexable`; the 230 shortest templated cards stay noindex for now.
...
**Description pass — COMPLETE (2026-08-29).** Every published program at a launched university now has an "About this program" description: **0 empty stubs** (was 507 at the start of the pass). Each is sourced from the official course page, house style, no em dashes, 60-240 words, validated by `scripts/program_descriptions_apply.mjs`. Tooling: `program_descriptions_pull.mjs` + `program_descriptions_apply.mjs` + `archive_programs.mjs` (log: `scripts/data/archived-discontinued.json`). Source of truth is `scripts/data/programs.json` via `export_programs.mjs`; ISR busted via POST /api/revalidate for `programs` after every batch.

- **~428 descriptions written across 19 batches**, ~21 discontinued/suspended programs archived along the way (Bond: Exercise and Sports Science, Business Data Analytics + Professional twin, Social Science, Master of International Relations, Master of Project Innovation, Master of BIM and Integrated Project Delivery, Master of Laws in Family Dispute Resolution + Graduate Certificate in Family Dispute Resolution, Bachelor of Exercise and Sports Performance, Bachelor of Health Transformation, Graduate Diploma of Psychological Science; Murdoch: Bachelor of Science B1317). Archived total in DB: 83 (incl. the earlier 62 combined-degree + 7 "(3 Year Program)" permutation rows).
- **AdSense-conscious approach (per user, 2026-08-29):** family-templated cards for the long tail (grad certs/diplomas anchored to their parent Master of X; honours-year cards; research-degree cards; pathway/enabling cards) but each carries real per-row substance — specific subjects, credit points, accreditation bodies, and honest limitations (e.g. "not a qualification to practise law", "not recognised as a teaching qualification", "not available to international students", "does not confer registration"). Nothing padded to hit word count.
- **Also fixed:** the 14 Bond "Master of X (Professional)" twins had `application_url` pointing at the generic sibling page; repointed each to its dedicated `bond.edu.au/program/master-of-<slug>-professional` page (all verified 200).
- **Data-quality items still flagged** (described, but worth a cleanup pass): Murdoch "Master of Applied Psychology" row name is ambiguous (covers both the Clinical m1156 and Professional m1238 streams — described as the two-stream family); Canberra "Doctor of Business Administration" is likely a duplicate of the "Professional Doctorate in Business Administration (Research)" (both described consistently, same source URL); Murdoch "Bachelor of Education" has no single matchable page (described as the named-specialisations family).

- **Structured IELTS/PTE backfill (2026-08-29):** all 424 stub programs had `ielts_overall` and every band NULL, so the "English Test Scores" box on every stub page showed the (often too-low) university default — e.g. UC health master's showing IELTS 6 when they need 7.0. Fixed **422 rows** via `scripts/apply_english_scores.mjs`: parsed `ielts_overall` + bands + `pte_overall` from each program's own `english_requirements` text, with **8 registration-gated programs overridden to match the sourced description** (Adelaide Clinical Exercise Physiology → 6.5 entry not 7.0; UC teaching degrees → 7.5; UC Nursing Practice → 7.0; UC Speech Pathology → 8.0; UC Foundation Studies → 5.5) and their `english_requirements` text aligned. 2 ELICOS/English rows left NULL (no minimum). Validator rejects band > overall and out-of-range values. Verified live: UC Master of OT now shows IELTS 7, Speech Pathology 8, CEP 6.5.
  - Also fixed the underlying fallback bug in `programs/[programId]/page.tsx`: it was passing `program.pte_overall ?? university.pte_overall` alongside `program.pte_listening ?? university.pte_listening`, so a program's own overall got shown next to the *university's* per-skill bands (e.g. "PTE 58 overall (L70 R65 W78 S70)"). Now IELTS and PTE are each taken as a whole set from the program when it has its own overall, else from the university, never mixed. The 229 backfilled rows now show a clean "PTE X overall" with no bands.
- **`admission_requirements` fix (2026-08-29):** the AI-import pipeline left vague/templated and sometimes contradictory admission text on the stub programs (e.g. UC Master of Counselling said "psychology, social science or a related field" while the sourced description and official page say "any discipline"). Fixed **~122 rows** via `scripts/apply_admission_requirements.mjs` (validates no em dashes): 100 with a vague generic value, 20 direct discipline-restriction contradictions vs the sourced description, 2 medical programs (UC Master of Medical Imaging / Nutrition and Dietetics), all rewritten to concise per-program statements consistent with the official course page / the description written in the pass. Also repaired 67 rows where a build-script escaping bug had stored `bachelor+s`/`Bond+s` instead of the apostrophe. **Not touched:** the 596 enriched programs (their admission text is per-program and was verified during enrichment) and the ~300 stub rows whose existing admission text is already specific and non-contradictory. Minor AR-vs-description gaps remain on some rows (AR omitting an IELTS band or a police-check note that shows elsewhere on the page) — low priority, not "stale", would need per-page re-verification.

- **Un-hide wave — DONE (2026-08-29), staged not blanket.** `isProgramIndexable(program)` in `public-programs.ts` gates both the page `robots` metadata and the sitemap `programEntries`: index if the program has a parsed `curriculum` (the 596 fully-enriched) OR an "About this program" description of **>= 100 words** (threshold lowered from 110 on 2026-08-30, commit `72f7729`, after a check found the 100-109w band is ~78 real degree pages — Master of Psychology, Master of Physiotherapy, Master of Pharmacy, CS majors, etc. — not thin content). The genuinely short templated cards **below 100 words** (grad-cert one-liners, short honours years, short pathway cards) stay `noindex, follow` and out of the sitemap, pending a later verification wave. Result at threshold 110: 790 program URLs in the sitemap (total ~1,067). Expected at 100: ~868. Verified in dev at 110: indexable pages `index` + in sitemap, sub-threshold pages `noindex` + absent.
  - Rationale for staged: the AdSense application window is ~2026-09-18. ~868 pages where the weakest is a 100+ word sourced summary with a citation is defensible; dumping all 1,020 including the 60-90 word templated cards right before an AdSense review is the "scaled content" risk. To go more or less conservative, change the word threshold in `isProgramIndexable` (or gate on `curriculum` only) — one-line change.
  - **USER ACTION:** after this deploys, resubmit `/sitemap.xml` in Google Search Console so the ~790 new program URLs get crawled. Optionally "Request indexing" on a few high-demand program pages.
...
## 27. Program description pass done + next-session plan (2026-08-29 / 30)

### Program pages — description pass complete, staged un-hide
See Section "Description pass — COMPLETE" above for the full record. Summary: every published program now has a sourced "About this program" description (0 empty stubs, was 507). `admission_requirements` fixed on ~122 stub rows against official pages; structured IELTS/PTE backfilled on 422 rows from `english_requirements`; a fallback-mixing bug in `programs/[programId]/page.tsx` fixed (IELTS/PTE now taken as a whole set from program-or-university, never mixed).

**Un-hide is live and staged:** `isProgramIndexable(program)` in `public-programs.ts` gates the page `robots` meta and the sitemap `programEntries` — index if the program has a `curriculum` OR a description of **>= 100 words** (lowered from 110 after checking the 100-109w band is real degree pages, commit `72f7729`). Sub-100-word templated family cards stay `noindex, follow` and out of the sitemap. **~868 program URLs** expected in the sitemap after `72f7729` deploys (was 790 at threshold 110).

**PENDING (do first next session):**
- ~~`git push` failing with 403 "denied to LamaRoman"~~ **RESOLVED 2026-08-30.** Not a token problem — wrong active `gh` account. Two accounts exist (`LamaRoman`, `zentaralabs`); repo is `zentaralabs/educational-website`. Fix: `gh auth switch --user zentaralabs` then `git push`. Both commits (`72f7729`, `876c288`) pushed + deployed. Sitemap verified live: **1145 total URLs / 868 program URLs** (was 1067 / 790), XML valid, HTTP 200. `programs` tag revalidated on prod.
- **STILL ON USER:** resubmit `/sitemap.xml` in **GSC** (property under romanlama314@gmail.com) and **Bing Webmaster Tools**. GSC had shown "Couldn't fetch" on 2026-08-29 for the 790-URL version — likely deploy churn; sitemap is valid.

### Product direction — assessment of the ChatGPT teardown (2026-08-30)

A long ChatGPT product teardown was reviewed. It scored the foundation 8/10 (fair) and its core thesis — "turn the data into a decision tool, not a directory" — is sound. **But it was working from a stale crawl** (couldn't fetch the live site) and much of what it recommends is already built: the quiz is now 8 inputs with redesigned result cards (Section 26), `/methodology` + Dataset schema + `/editorial-policy` + `/about` (real named operator) + per-page `LastVerified` + bylines all exist, `/best` collections already do "best for X", comparison + country pages exist, the 2026-08-29 homepage rework added the "What are you looking for?" task grid + "Popular right now" row.

**What's genuinely not done and worth doing (in priority order, all aligned with the AdSense + SEO phase):**

1. **Per-datapoint verification surfacing.** `LastVerified` sits at page level / footer. Put "verified {date} · source" *inline* next to the high-consequence facts — tuition, deadline, entry requirements — on university and program pages. Programs already carry `last_verified_at` + `source_url` per row (100% have source_url); universities/deadlines have their own. Extend `LastVerified` or add a small inline `VerifiedFact`. Keep it restrained (not a badge farm). Also surface a condensed "Why trust Where To Apply" (4 bullets: official sources / manually verified / last-verified dates / independent) on the homepage + university pages, linking to `/methodology`. **This is the single best pre-AdSense move** — it directly strengthens the "high-value, maintained content" signal.

2. **Course/subject pages as a visible cross-university comparison table.** Today `/study/[slug]` has editorial prose + a `Course` ItemList in JSON-LD (schema, not a rendered table). Build the visible table: for a subject, every published program matched to it, columns = university, level, duration, tuition (intl), entry requirement, next deadline, scholarships count, link. This answers a query the universities' own pages can't ("all 20 unis offering Master of Data Science, compared"). Constraint: only 6 subjects have curated `SUBJECT_CONTENT` (the rest are templated-fallback/noindex) — the table itself may be enough added value to index more, but decide that deliberately; don't ship 19 thin tables. Reconcile with the existing `/best/[subject]` collections rather than duplicating. **Biggest SEO lever, but larger — scope it properly.**

3. **Homepage: make the finder the unambiguous primary CTA.** Right now `/quiz` is one task-card among equals. Promote it to the hero's primary action ("Find my universities"), keep "browse all universities" as clearly secondary. Cheap; do it alongside #1.
...
### Priority 2 — subject pages as a cross-university comparison table (DONE 2026-08-30)

- New `src/components/site/SubjectComparisonTable.tsx` — server component, rendered on **every** `/study/[slug]` page. Every published program matched to the subject, one row: **University** (link + city) · **Program** (link to program page) · **Level** · **Duration** · **Tuition / yr** (intl) · **IELTS**. Sorted cheapest-first (unpriced rows last), then university. Scrolls inside its own `overflow-x-auto` container (`min-w-[40rem]`); no page-body overflow at 375 or desktop.
  - **Cap: 60 rows.** Subjects over 60 programs (Business 199, Nursing & Health 136, Psychology 65, IT 63) show "The 60 lowest-tuition {subject} degrees, of N in our dataset"; the other ~14 subjects show the full set ("Compare all N …"). `CAP` constant, one-line change.
  - `formatDuration`: whole/half years → "2 years" / "1.5 years"; odd accelerated lengths → months ("4 months", "16 months"). `shortCity` splits on `,` **and** `(` so "Multi-campus (Sydney, …)" renders as "Multi-campus".
  - `sr-only` `<caption>` for a11y / SEO.
- Query: `public-subjects.ts` `PROG_SELECT` + `SubjectProgram` now carry `ielts_overall` (program) with fallback to the university's institutional minimum; also added `universities.ielts_overall` to the join.
- Replaced the old "Most affordable {subject} programs" 15-row list and the "All universities offering {subject}" pill list — the table supersedes both. `subject.universities` still feeds the `Course` `ItemList` JSON-LD (unchanged: one `Course` per university on curated pages).
- **Indexing unchanged / deliberate:** the table renders on all 18 subject pages (users + internal links) but only the 11 with curated `SUBJECT_CONTENT` stay indexed; the ~7 templated-fallback subjects remain `noindex, follow`. Whether the table alone justifies indexing more is a later call — the gate is one line in `generateMetadata`.
- **Data-quality items noticed, not fixed** (out of scope, worth a curation pass): the `nursing-and-health-sciences` bucket bundles nursing + all allied health (136 rows) — arguably too broad to be one useful comparison; Bond is over-represented in several subjects (Actuarial Science / Data Analytics variants); some `degree_level` values are mislabeled (e.g. a coursework master's tagged "PhD").
- Verified in dev: data-science (30 rows, full), business (60-row cap + note), nursing (136→60), computer-science (34); tsc + eslint clean; no server errors.

### Priority 2 follow-up — data-quality curation pass (2026-08-30)

Spawned from the items the comparison tables surfaced. Full review done; only the low-risk, unambiguous fixes applied.

**Applied (7 rows in `scripts/data/programs.json`, commit this pass) — USER MUST RUN `node scripts/seed_programs.mjs --commit` then `POST /api/revalidate` with `{"table":"programs"}`:**
- `degree_level` `PhD` → `Graduate` on 2 research master's mislabeled as doctorates: Bond *Master of Laws (by Research)*, UC *Master of Information Sciences (Research)*. (Only 2 of the 14 published `PhD`-level rows were actually wrong; the rest — DBA, EdD, DIT, DPsych, professional doctorates — are genuine doctorates and "PhD" is an acceptable coarse label. No new enum value added.)
- `subject` `political-science-and-international-relations` → `education` on Bond *Master of Arts (TESOL)* (teaching qualification, clearly misfiled).
- `subject` `psychology` → `law` on Bond's 4 criminology rows (*Bachelor of Criminal Justice and Criminology*, *Master of Criminology*, *Master of Criminology (Professional)*, *Graduate Certificate in Criminology*) — matches where Murdoch's criminology already sits; `psychology` was the worst possible home.

**Reviewed, NOT actioned (deliberate — scoped follow-ups, not quick fixes):**
1. **Split `nursing-and-health-sciences` (136 programs).** It is a coherent "Nursing & Health Sciences" bucket but too broad for one comparison table. Natural split: `nursing` (~40: Bachelor/Master of Nursing, midwifery), `allied-health` (~50: physio, OT, speech, exercise physiology, chiro, optometry, pharmacy, paramedicine, dietetics, medical imaging), `public-health` (~20: MPH ×7, health admin/policy, infectious disease), plus medicine (~6) and generic health science (~15) folded into allied-health. This is a **content project**, not a data edit: needs 2-3 new `SUBJECT_CONTENT` entries, `SUBJECT_BEST_PAGES` update, an old-slug redirect in `study/[slug]`, sitemap, and re-verification. The 60-row table cap already tames the UX in the meantime.
2. **Bond "(Professional)" twins are legit** — verified in the 2026-08-29 description pass (URLs repointed to dedicated `bond.edu.au/program/...-professional` pages, all 200). Bond genuinely runs standard + Professional (placement) variants. Not cruft; not deleting.
3. **The real over-representation is Adelaide's colon-major rows** — `Bachelor of Business: <13 majors>`, `Bachelor of Commerce: <5>`, `Bachelor of Computer Science: <3>`, `Bachelor of Information Technology: <4>` are one degree each listed as many program rows. Consolidation would help the tables but each row now has its own description + source (description pass), so it needs a careful merge, not a bulk archive. Follow-up.
4. **Social work** (~7 rows) is scattered across `psychology` and `nursing-and-health-sciences` with no good home — left as-is (a new `social-work` subject would be sub-threshold and noindex, not a win).
5. **Criminology is still split** `law` vs `political-science-and-international-relations` (the "Criminology and Justice" bachelors at Griffith/ECU/USC stay in poli-sci). Lower priority; the Bond move above at least removes the worst inconsistency.

### Homepage polish + search tuning (2026-08-30)

From a "make the homepage say universities + courses + visas, surface visa in search" discussion. Applied the low-risk subset:
- **Hero stat pill:** `56 universities · 221 deadlines tracked` → `56 universities · 1,000+ courses · 221 deadlines` (`getHomepageStats` now also returns `programCount`, floored to the nearest 100 with a `+`; ~1,020 published programs). Wraps to two lines inside the pill on 375px, acceptable.
- **Hero subhead:** added "courses" and "visa pathways" to the list ("Compare universities, courses, application deadlines, tuition, entry requirements, scholarships, and visa pathways for Australia").
- **H1 unchanged** ("Where should you apply?") — kept the decision-framed hook that pairs with the quiz CTA; scope now lives in the subhead + pill instead.
- **`AnimatedPlaceholder` phrases** were stale US/UK ("MIT", "Stanford", "Scholarships in Canada", "MBA in the UK") → Australia-relevant and search-advertising: "Master of Data Science", "student visa 500", "University of Melbourne", "485 graduate visa", "scholarships for international students", "nursing degrees in Brisbane".
- **Search already covered visas** — the fix was ranking/order: `/search` sections reordered to **Universities → Visas → Programs → Guides → Scholarships → Blog** (visa is specific + only 12 rows, never a wall). And `graduate` / `postgraduate` / `undergraduate` added to the search `STOPWORDS` — they matched every "Graduate Certificate/Diploma" row and buried the real hit. "485 graduate visa" now returns the 485 visa first (was: ~30 program rows first). `bachelor`/`master` deliberately kept (they usefully narrow a course search).
...
**`/visas/training-visa-407` reference page — DONE 2026-09-02.** Added to `scripts/seed_visas.mjs` (category `employer-sponsored`, sits with 482/186/494; the label is a slight stretch but that is the right comparison neighbourhood and the card copy leads with "training, not a job"). Facts verified against Home Affairs: 3 training types, up to 2 years, from AUD 535, temporary-activities sponsor + nomination, family work 40 hours a fortnight, no PR pathway, no extension. The 407-vs-482 guide now links to it. Also added a keyword-pass tweak to the guide (title, `## at a glance` heading, "does the 407 lead to PR?" FAQ).
