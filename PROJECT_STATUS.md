...
- **Progress as of 2026-08-29: 61 written** (Bond ~46 undergrad + masters, Adelaide ~15), **9 discontinued programs archived** (Bond: Exercise and Sports Science, Business Data Analytics, Social Science, Master of International Relations; more expected). **377 empty-published stubs remain.**
- Resume: `node scripts/program_descriptions_pull.mjs "<University>"`, work the bachelor/master degrees first (skip "(Professional)" twins, standalone grad certs/diplomas, honours-only rows), fetch each official course page, write to `scripts/data/_w.json` as `[{id, description, source_url, duration_years?}]`, apply, export, revalidate, commit. Remaining priority unis: finish Bond masters, Adelaide (mostly done its degrees), then Murdoch (118) and Canberra (119).
