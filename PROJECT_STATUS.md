...
- **Progress as of 2026-08-29: 374 written** (Bond ~114, Adelaide 70, Murdoch 79, Canberra 111); Canberra + Murdoch bachelor sweeps picked up cleanly-matchable degrees missed earlier (Canberra: Arts, Commerce (Business Economics), Digital Design (Digital Media), Early Childhood Education, Engineering Technology, Medical Imaging, Occupational Therapy, Science; Murdoch: Arts, Creative Media, Applied Social Care). Bond's 14 "Master of X (Professional)" twins now described as the standard master's plus a compulsory 20-week Work Integrated Learning component (Australian internship placement + professional portfolio) in a final 5th semester, international-students-only. Adelaide degree-level stubs now essentially cleared (remaining Adelaide stubs are honours-only, combined-degree, or Master of Engineering / Bachelor of Music / Bachelor of Teaching rows with no cleanly matchable single course page), **17 discontinued programs archived** (Bond: Exercise and Sports Science, Business Data Analytics + Professional twin, Social Science, Master of International Relations, Master of Project Innovation, Master of BIM and Integrated Project Delivery, Master of Laws in Family Dispute Resolution, Bachelor of Exercise and Sports Performance, Bachelor of Health Transformation; Murdoch: Bachelor of Science B1317). **126 empty-published stubs remain** (mostly the 116 grad certificate + grad diploma rows, plus ~10 flagged minimal-card rows — see long-tail pass below).
- Also fixed (2026-08-29): the 14 Bond Professional twins had `application_url` pointing at the generic sibling course page (`master-accounting` etc.); repointed each to its dedicated `bond.edu.au/program/master-of-<slug>-professional` page (all verified 200). Source citation `[1]` on each twin page already points to the same dedicated page. Export + revalidate done.
- **Long-tail pass IN PROGRESS (2026-08-29):** user wants the ~212 remaining stubs described too, but "in the best interest of AdSense approval, not filling AI content blindly" — so: family-templated batches where the templated card carries real per-row substance (specific subjects, accreditation, honest limitations), decline to a minimal card where nothing genuine can be said, archive junk rows. Un-hide (drop noindex + add programs to sitemap) happens as ONE wave AFTER all descriptions are done, gated on description presence/quality.
  - **Batch 10 done:** 7 real Canberra coursework master's from the "needs eyes" bucket (Building & Construction Information Management, Business Informatics, Counselling, Master of Education, Master of Education Studies, Educational Leadership and Management, TESOL & FLT).
  - **Batch 11 done:** 11 real Murdoch coursework master's whose stub URL was the generic `study/courses` — found each real course page (m-code), wrote sourced cards: Professional Communication, Sustainable Development, Health Care Management, Biosecurity, Community Development, Creative Arts Therapies, High Performance Sport, Decarbonisation Energy & Sustainability Management, Food Security, Health Administration Policy & Leadership, Infectious Disease Surveillance & Control. **194 stubs remain.**
  - **Batch 12 done:** 6 name-mismatch / generic degrees — Adelaide Bachelor of Music (single degree, 4 majors), Bachelor of Teaching (Primary Honours), Master of (Professional) Engineering (9 specialisations); Canberra Bachelor of Creative Industries (TAFE Qld), Bachelor of Secondary Education, Bachelor of the Built Environment.
  - **Batch 13 done:** 19 research degrees — Bond/Murdoch/Canberra PhD, Bond MPhil + Doctor of Legal Science + Master of Laws (research) + Professional Doctorates of OT (entry-to-practice) & Physio (post-professional), Murdoch DBA/DEd/DPsych Clinical + RMT umbrella + Accelerated RMT + field RMTs (Education/Exercise Science/Laws), Canberra Master of Research + Master of Info Sciences (Research) + Professional Doctorate in Business Administration. Still minimal + flagged: Murdoch Doctor of IT, Doctor of Veterinary Medical Science, Master of Philosophy; Canberra Doctor of Business Administration (generic URLs / redundant).
  - **Batch 14 done:** 21 honours — 8 standalone 4-yr integrated-honours degrees sourced individually (Adelaide Eng Electrical/Environmental, Clinical Exercise Physiology, Interior Architecture; Canberra Building & Construction Management, Engineering) + 13 one-year add-on honours-year cards (Bond Health Sciences ×2, Canberra Arts&Design / Business Government&Policy / Health Sciences / Psychology(APAC 4th yr) / Science&Technology, Murdoch Arts/Business/Communication/Creative Media/Criminology/Psychology/Science/Law honours).
  - **Batch 15 done:** 22 pathway/enabling/diploma — Bond College diploma family (Arts/Built Env/Business/Creative Design/Health Sci/Legal Studies) + Foundation Program + Diploma Prep + Grad Dip Psychology (Bridging); UC College diploma family (Business/Communication/Design/Health/IT/Science) + International Foundation Studies; Murdoch K-Track (First Nations), OnTrack Sprint/Flex, TLC, Waardong (First Nations vet), FlexiTrack High. **126 stubs remain.**
  - Deferred/flagged (still minimal card): Murdoch "Master of Applied Psychology" (ambiguous — m1156 Clinical vs m1238 Professional, needs a name fix); Murdoch Doctor of IT / Doctor of Vet Medical Science / Master of Philosophy; Canberra Doctor of Business Administration; ELICOS rows (Bond "English", Canberra ELICOS — genuinely just English tuition); Canberra UC Connect / UC Preparation Scheme / Grad Cert in Academic Foundations; Murdoch Diploma of Sport and Business (generic URL); Bond/Murdoch "Bachelor of Education" (no single matchable page).
  - **Batch 16 done (Bond grad cert/dip):** 28 written as family cards anchored to the parent Master of X (Bond structure: grad cert = 4 subjects/~1 semester, grad dip = 8 subjects/~2 semesters, full articulation with credit). Distinct ones handled properly: Graduate Diploma in Legal Practice (GDLP → admission as a lawyer), Bond-BBT Global Leadership (Japan partnership, no student visa), nutrition family (Dietitians Australia accred, master's = APD pathway), building surveying family (RICS, certifier pathway). Honest limits flagged where the Bond page says "not available to international students" (Business, BIM, Health Systems, Environmental Consulting, Evidence Based Practice). **2 more archived** — Graduate Certificate in Family Dispute Resolution + Graduate Diploma of Psychological Science, both suspended by Bond from Jan 2025 (19 discontinued total).
  - **Batch 17 done (Canberra grad cert/dip):** 42 written as family cards (UC structure: grad cert = 12 CP / 4 units / ~8mo PT, grad dip = 24 CP, master = 48 CP, articulated). Distinct handling: Graduate Certificate/Diploma in Law (NOT practice qualification → JD is), Ultrasound Studies cert + Medical Ultrasound diploma (ASAR sonographer accreditation, diploma is the accrediting step), Country as Teacher (First Nations pedagogy, cohort entry), Nutritional Science diploma (science not dietetics registration), Undergraduate Certificate in Education (UG-level, not a teaching qual). Also wrote Graduate Certificate in Academic Foundations (study-skills bridging).
  - **Still to do: ~42 Murdoch grad cert/dip — all have generic `study/courses` URLs, need real m/c/g-code course pages found (Murdoch nests grad cert → grad dip → master too). Then the ~10 flagged minimal-card rows are the permanent tail. 54 stubs left.**
- Per-batch workflow: `node scripts/program_descriptions_pull.mjs "<University>"` → write `scripts/data/_w.json` as `[{id, description, source_url, duration_years?}]` → `node scripts/program_descriptions_apply.mjs scripts/data/_w.json --commit` → `node scripts/export_programs.mjs` → POST /api/revalidate for `programs` → commit.
