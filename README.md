...
**Structured program/course data added** (migrations `0004_add_programs.sql` and `0005_add_subjects.sql`, applied directly against production via a temporary `DATABASE_URL` since this project has no linked Supabase CLI project yet). New `programs` table holds per-university degree offerings (e.g. "Bachelor of Computer Science") — name, degree level, subject, duration, and an optional per-program tuition override that falls back to the university's `tuition_international` when unset. Subject is a controlled vocabulary (`subjects` table, seeded with 20 broad categories, managed in Settings alongside the other lookups) rather than free text, so it's actually filterable — closes the gap `PROJECT_STATUS.md` Section 12 originally flagged. Managed from the university edit screen's "Academic" tab (`/admin/universities/[id]`); shown publicly on each university profile under "Academics".

**Blog added** (migration `0006_add_blog_posts.sql`) — a new `blog_posts` table, kept separate from `guides` since blog posts are chronological/dated (need a real `published_at` for ordering) while guides are evergreen reference content; mixing them would force every guide list/filter view to branch on category. Admin: `/admin/blog` (list + editor, mirrors the guide editor's markdown preview but without the QA checklist, plus excerpt/tags fields). Public: `/blog` (index, newest first) and `/blog/[slug]` (detail, with `BlogPosting` schema.org JSON-LD). Both nav bars link to it now.
